...
class HeadlessTriangleApplication {
public:
    void run() {
        initVulkan();
        renderOnce();
        saveImage("triangle.ppm");
        cleanup();
    }
...
private:
    ...
    void createInstance() {
        ...
        appInfo.pApplicationName = "Headless Triangle";
    ...
    void createGraphicsPipeline() {
        ...
        rasterizer.cullMode = VK_CULL_MODE_NONE; // 先关掉剔除，最稳
    ...
    void saveImage(const std::string& filename) {
        ...
        if (vkMapMemory(device, readbackBufferMemory, 0, VK_WHOLE_SIZE, 0, &data) != VK_SUCCESS) {
            throw std::runtime_error("failed to map readback buffer memory!");
        }

        auto* pixels = reinterpret_cast<unsigned char*>(data);
        ...
        for (int y = static_cast<int>(HEIGHT) - 1; y >= 0; --y) {
            for (uint32_t x = 0; x < WIDTH; ++x) {
                ...
                file.put(static_cast<char>(pixels[i + 0]));
...
int main() {
    HeadlessTriangleApplication app;
